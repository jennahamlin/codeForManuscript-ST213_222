...
```{r, st1 ANI, echo = FALSE, warning =  FALSE, message = FALSE}
########################
#    INTRA - ANI       #
########################
...
## 99.82
ani1Mean <- meanANI('ani-mummer1.txt')
```
...
casSum <- function(df, st) {
  pattern <- c("CAS", "elGatoSTCalls")
  cas_columns <- grep(paste(pattern, collapse = '|'), names(df), value = TRUE)
  cas_columns <- df %>% dplyr::select(all_of(cas_columns))
  
  if(ncol(cas_columns) == 5) {
    
    #print('Good, the number columns matches the expected number of columns (n = 4)')
    cas_columns %>%
    dplyr::group_by(elGatoSTCalls) %>%
      filter(elGatoSTCalls == elGatoSTCalls) %>%
      summarise(totalIoslate = n())
    
    casCount <- cCount(df, st) %>%
...
cas213Zero <- casSum(isoLoc, 213) # This is zero 
...
moreThan <- isoLoc %>% 
  ...
  dplyr::group_by(elGatoSTCalls) %>%
  dplyr::summarise(counts = sum(Total >=2, na.rm = T))

## Isolates with more than one unique cas system are 
## type IC and IIB, get the count 
mostTwo <- isoLoc %>% 
  dplyr::select(
    elGatoSTCalls, CAS.TypeIA, CAS.TypeIC, CAS.TypeIF, CAS.TypeIIB) %>% 
  dplyr::filter(elGatoSTCalls != 1742 & elGatoSTCalls != 227 & 
                  elGatoSTCalls != 276 ) %>%
  dplyr::mutate_all( function(x) as.numeric(as.character(x))) %>%
  dplyr::filter(CAS.TypeIC != 2) %>%
  dplyr::mutate(Total = 
                  select(., CAS.TypeIC,CAS.TypeIIB)
                %>% rowSums(na.rm = TRUE)) %>%
  dplyr::group_by(elGatoSTCalls) %>%
