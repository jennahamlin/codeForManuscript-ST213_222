...
To generate a comparative genomic map, CGView was used to compare the PacBio completed whole-genome assemblies [@stothard_circular_2005], which operates BLAST version 1.1.0 for the comparisons. Percent identity is calculated for the three isolates, with 100, 90, or 70% matching identity bins, and based on the Toronto-20052 genome assembly (NZ_CP012019.1, ST222). To evaluate genomic synteny, we ran progressiveMauve [@darling_progressivemauve_2010] for the three PacBio isolates, and the same reference sequence used in the CGView analysis (NCBI Reference Sequence: NZ_CP012019.1, ST222).
...
**Supplemental Table 6:** Genes present only in the C217 (NZ_CP117814.1, ST213) PacBio genome compared with the other PacBio genomes (C175, ST222 and C177, ST213) using the Toronto-20052 genome (NZ_CP012019.1, ST222) as a reference on which the start and stop are based. Note that this list excludes all possible genes missing in ST213 compared to ST222 and only contains those genes in the C217 isolate for the region located at 188 kb. Column headers: onlyInC217 – Presence in C217, start..stopFrom-NZ_CP117814.1 - positional coordinates for each gene based on the NZ_CP117814.1 RefSeq genome (ST213, C217),start..stopFrom-NZ_CP012019.1 - positional coordinates for each gene based on the NZ_CP012019.1 RefSeq genome (ST222), locusTagFrom-CP117814.1 - locus tag of each gene based on the NZ_CP117814.1 RefSeq genome (ST213, C217),  locusTagtFrom-NZ_CP012019.1 - locus tag of each gene based on the NZ_CP012019.1 RefSeq genome (ST222), productFrom-NZ_CP117814.1 - concordant protein-coding gene product from the NZ_CP117814.1 RefSeq genome (ST213, C217),  productFrom-NZ_CP012019.1 - concordant protein-coding gene product from the NZ_CP012019.1 RefSeq genome (ST222),  proteinIDFrom-NZ_CP117814.1 - corresponding NCBI protein ID from the NZ_CP117814.1 RefSeq genome (ST213, C217), proteinIDFrom-NZ_CP012019.1 – corresponding NCBI protein ID from the NZ_CP012019.1 RefSeq genome (ST222), similiarToAASequence:RefSeq-NZ_CP117814.1 - protein IDs using a single non-redundant protein accession number from the NZ_CP117814.1 RefSeq genome (ST213, C217), similiarToAASequence:RefSeq-NZ_CP012019.1 - protein IDs using a single non-redundant protein accession number from the NZ_CP012019.1 RefSeq genome (ST222), aminoAcidSequence- NZ_CP117814.1 - amino acid sequence from the NZ_CP117814.1 RefSeq genome (ST213, C217), aminoAcidSequence- NZ_CP012019.1 - amino acid sequence from the NZ_CP012019.1 RefSeq genome (ST222).

**Supplemental Table 7:** : Genes absent in C177 (ST213) and C217 (ST213) PacBio genomes compared with the other PacBio genome (C175, ST222) using the Toronto-20052 genome (NZ_CP012019.1, ST222) as a reference. The start and stop are based on the NZ_CP012019.1 genome (ST222). Note that this list excludes all possible genes missing in ST213 compared to ST222 and only contains those not found in the missing segments at ~90 kb and ~950 kb for ST213 (C217). Column headers: missingInST213 – absence in C177 and C217 genomes compared to NZ_CP012019.1 RefSeq genome (ST222), start..stopFrom-NZ_CP012019.1 - positional coordinates for each gene based on the NZ_CP012019.1 RefSeq genome (ST222), locusTagtFrom-NZ_CP012019.1 - locus tag each gene based on the NZ_CP012019.1 RefSeq genome (ST222), productFrom-NZ_CP012019.1 - concordant protein-coding gene product from the NZ_CP012019.1 RefSeq genome (ST222), proteinIDFrom-NZ_CP012019.1 – corresponding NCBI protein ID from the NZ_CP012019.1 RefSeq genome (ST222), similiarToAASequence:RefSeq-NZ_CP012019.1 - protein IDs using a single non-redundant protein (WP_) accession number from the NZ_CP012019.1 RefSeq genome (ST222), aminoAcidSequence-NZ_CP012019.1 amino acid sequence from the NZ_CP012019.1 RefSeq genome (ST222).

**Supplemental Table 8:** Presence or Absence from Roary pan-genome analysis, which lists each gene and the samples which that gene is present in. Column headers are standard output from Roary but are as follows: Gene - the most frequently occurring gene name from the sequences in the cluster. If there is no gene name a generic unique name (group_XXX) is given, Non-unique gene name - where sequences with the same gene name have ended up in different groups. It might be because of split genes or missed annotation, Annotation - most frequently occurring functional annotation from the cluster is used, No. isolates - Number of isolates represented in the cluster, No. sequences - Number of sequences in the cluster. Avg sequences per isolate - Average number of sequences per isolate. This is normally 1. If this is greater than 1 then there is over clustering and the paralogs couldn’t be split, Genome fragment - evidence at the contig level that the genes are linked, Order within fragment - when combined with the genome fragment this gives an indication of the order of genes within the graph, Accessory Fragment - where core genes are excluded and there is evidence at contig level that the genes are linked, Accessory order with fragment - when combined with the Accessory fragment this gives an indication of the order of genes within the accessory graph, QC - Comments on the quality of the cluster. Miss predictions are noted as are single genes on single contigs that could be evidence of low level contamination, Min group size nuc	- Minimum sequence length in nucleotides of the cluster, Max group size nuc	- Maximum sequence length in nucleotides of the cluster, Avg group size nuc - Average sequence length in nucleotides of the cluster, All remaining columns is presence and absence of genes for each sample.

**Supplemental Table 9:** This table lists the metadata for the clinical isolates selected at random to generate the SNP phylogeny of sequence types. Metadata includes an alternative identifier (sampleID), sequence type (ST), location for collection of isolates (State), and date of collection (Year). 

**Supplemental Table 10:** This table shows the presence/absence of all locus tags associated with the Toronto-20052 genome  (NZ_CP012019.1, locus tag: lpt) in all isolates examined here. The first column contains all lpt locus tags related to the Toronto-20052 genome (Toronto-LPT-Tag-ID), and each subsequent column is for an individual genome using the isolateAccession identifiers. If there is a blank, then that indicates a lpt locus tag was not found within that genome. Note that some lpt locus tags are seen more than once in the lpt locus tag column (Toronto-LPT-Tag-ID); this is to account for those locus tags that are found more than once in an isolate (e.g.,  locus_tag=lpt_00295). 	
